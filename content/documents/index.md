...
[Uncertainty Quantification for Motor Imagery BCI - Machine Learning vs. Deep Learning poster](/poster_uqbci_ml_dl.pdf), and a [corresponding youtube video](https://youtu.be/LddnmJ4SgE0), presented at Decoding the Brain Time Series session at IEEE Machine Learning for Signal Processing, with Joris Suurmeijer. We demonstrated that classical ML methods for BCI have better uncertainty estimation than Deep Learning methods. We show that MDRM (Minimum Distance to Riemannian Mean) does not have well-calibrated uncertainties, and propose a solution to this.

[Why Large Language Models can Secretly Outperform Embedding Similarity in Information Retrieval poster](/poster_llms_vs_embedding.pdf), presented by Matei Benescu at the ACL-SRW. We show that there is a core limitation in Information Retrieval based on embedding similarity, where similarity is conflated with relevance. We demonstrate that LLMs can resolve this, though that this does not always show as improvement in human annotated datasets.

[A continuous control paradigm for Motor Brain-Computer Interfaces](/poster_ncm_motor_control.pdf), presented at the Neural Control of Movement conference, showing ongoing work targetted at developing a Motor BCI that uses continuous control instead of single-movement control. We present the paradigm and the collected dataset, but are still finalising models and including uncertainty quantification as ongoing work.
